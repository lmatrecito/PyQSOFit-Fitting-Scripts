...
class QSOFit():
    ...
    def Fit(self, name=None, nsmooth=1, and_mask=False, or_mask=False, reject_badpix=True, deredden=True, wave_range=None,
            ...
            n_pix_min_conti=100, param_file_name='qsopar.fits', MC=False, MCMC=False, save_fits_name=None, use_ppxf=True,
            nburn=20, nsamp=200, nthin=10, epsilon_jitter=1e-4, linefit=True, save_result=True, plot_fig=True, save_fig_path=path2, save_fits_path='.',
    ...
    def line_prop_from_name(self, line_name, line_type='broad', ln_sigma_br=0.00142):
        """
    ...
    def line_prop(self, compcenter, pp, linetype='broad', ln_sigma_br=0.00142):
        """
    ...
    def plot_fig(self, save_fig_path=path2, broad_fwhm=1000, plot_line_name=False, plot_legend=True, ylims=None, plot_residual=True, show_title=True, plot_br_prop=False):
        """Plot the results
        ...
        """if self.linefit == True:
            ...
            for p in range(len(self.gauss_result)//(mc_flag*3)):
                ...
                #axn[1].plot(wave_eval, line_single, color=color, zorder=line_order[color])
...
# LIZA ADDED THE FOLLOWING BECAUSE WE DISLIKE THE SECOND ROW, WE DONT NEED IT, 11/29/2022 --------------------------------------------------------------------------------------------------
        ...
        self.f_line_br_model = np.zeros_like(self.wave)
        lines_total = np.zeros_like(wave_eval)
        line_order = {'#146746': 3, '#DB1D1A': 7}  # Ensure narrow lines plot above the broad lines
        ...
        # Supplement the emission lines in the first subplot
        ax.plot(wave_eval, lines_total + f_conti_model_eval, '#244BF9', label='overall fit', zorder=line_order[color])
...
# END LIZA EDITS ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        if self.wave_mask is not None:
            ...
            for j, w in enumerate(self.wave_mask):
                ...
                ax.axvspan(w[0], w[1], color='grey', alpha=0.2)
                
                # Plot avoiding drawing lines between masked values
                if j==0:
                    mask = self.wave_prereduced < w[0]
                    ax.plot(self.wave_prereduced[mask], self.flux_prereduced[mask], '#14080E', label=' SDSS data', lw=1, zorder=2)
                    #ax.plot(self.wave_prereduced[mask], self.err_prereduced[mask], 'gray', label='error', lw=1, zorder=1)
                if j==len(self.wave_mask) - 1:
                    mask = self.wave_prereduced > w[1]
                    ax.plot(self.wave_prereduced[mask], self.flux_prereduced[mask], '#14080E', lw=1, zorder=2)
            ...
            ax.plot(self.wave_prereduced, self.flux_prereduced, '#14080E', label='SDSS data', lw=1, zorder=2)
            
            # Residual
            if plot_residual == True:
                if self.linefit == True:
                    ax.plot(self.wave, self.line_flux - self.f_line_model, 'darkgray',
                            label='residuals', linestyle='dotted', lw=1, zorder=3)
                else:
                    ax.plot(self.wave, self.flux - self.f_conti_model, 'darkgray',
                            label='residuals', linestyle='dotted', lw=1, zorder=3)
        ...
        # Host decomposition
        # LM COMMENTED OUT FOLLOWING AND CHANGED TO SELF.DECOMPOSE 
        if self.decompose_host == True and self.decomposed == True:
        #if self.decompose_host == True and self.decompose == True:
        ...
        # Line legend hack
        ax.plot([0, 0], [0, 0], '#DB1D1A', label='broad components', zorder=5)
        ax.plot([0, 0], [0, 0], '#146746', label='narrow components', zorder=5)
        
        # Continuum results
        ax.plot(wave_eval, f_conti_model_eval, '#00A0CC', lw=2, label='FeII+continuum', zorder=7)
        ...
        ax.plot(wave_eval, self.PL(wave_eval, pp) + self.F_poly_conti(wave_eval, pp[11:]), color='#EDB007', lw=2, label='continuum', zorder=9)
        ...
        if self.linefit == True:
            ...
            mask_outliers = np.where(r < 3*mad, True, False)
            plot_top = np.max(self.flux[mask_outliers])+5
        ...
        if plot_legend == True:
            ax.legend(loc='best', frameon=False, ncol=3, fontsize=10)
