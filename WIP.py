...
for i in range(len(source)):
    ...
    paths = path_sources+source[i]+'/'+'Fit Data/'
    path.append(paths)
    # Path for saving results from this code
    paths2 = path_sources+source[i]+'/'+'Line Profile Plots/'
    path2.append(paths2)
...
for i in range(len(path)):
    ...
    bls = np.load(path[i]+source[i]+'_BLData.npy')                        # fitted BL data
    bl.append(bls)
    bl_profiles = np.load(path[i]+source[i]+'_BLSpectrum.npy')       # cleaned BL spectrum
    bl_profile.append(bl_profiles)
    nls = np.load(path[i]+source[i]+'_NLData.npy')                        # fitted NL data
    nl.append(nls)
    data_contFeII_subs = np.load(path[i]+source[i]+'_DataCFeII.npy')    # cleaned spectrum
    data_contFeII_sub.append(data_contFeII_subs)
    datas = np.load(path[i]+source[i]+'_Data.npy')             # flux from SDSS .fits file
    data.append(datas)
    wavelengths = np.load(path[i]+source[i]+'_Wavelength.npy')
    wavelength.append(wavelength)
    redshifts = np.load(path[i]+source[i]+'_z.npy')
    z.append(redshifts)
...
for i in range(len(path)):
    ...
    # MUCH easier
    # BL Data
    bl_matrices = np.vstack((wavelength[i], bl[i])).T
    bl_datas = pd.DataFrame(bl_matrices, columns=['Wavelength','Flux'])
    bl_data.append(bl_datas)
    # NL Data
    nl_matrices = np.vstack((wavelength[i], nl[i])).T
    nl_datas = pd.DataFrame(nl_matrices, columns=['Wavelength','Flux'])
    nl_data.append(nl_datas)
    # Max (used for plotting)
    data_maxes = np.amax(data_contFeII_sub[i])
    data_max.append(data_maxes)
    # Cleaning data to be used for calculating kurtosis, asymmetries, and centroid
    # and C80 velocity shifts
    clean_data = data_contFeII_sub[i] - nl[i]
    clean_matrices = np.vstack((wavelength[i], clean_data)).T
    cleaned_datas = pd.DataFrame(clean_matrices, columns=['Wavelength','Flux'])
    cleaned_data.append(cleaned_datas)
...
def vs_calcs(wave_range, nl_range, bl_data, nl_data, wavelength, bl, nl, 
    ...
    plt.title(source[i]+line_plot+r' CDF', fontsize=30)
    ...
    plt.savefig(path2[i]+source[i]+'_'+line+'_CDF.pdf')
    ...
    plt.savefig(path2[i]+source[i]+'_'+line+'_Profile-VSCalcs.pdf')
    ...
    plt.tick_params(which='major', length=12, width=1)
    plt.legend(fontsize=12)
    plt.savefig(path2[i]+source[i]+'_'+line+'_VProfile.pdf')
    ...
    with open(path2[i]+source[i]+'_'+line+"_VShifts.txt","w") as f:
        print(" ", file=f)
...
print('Data Analysis Finished in : '+str(np.round(end-start))+'s')
